...
themes = {
	pikachu: {
		...
		background: {
			...
			image: function(ctx, width, height) {ctx.globalAlpha = 0.7; let image = document.getElementById("pikachu_background"); image.onload = function() {ctx.drawImage(image, 0, 0, width, image.height, 0, 0, width, height)}}
		},
		player: {
			color: "magenta",
...
class Game {
	...
	step(dt) {
		...
		for (let i = 0; i < this._blocks.length; ++i) {
			this._blocks[i][1] += dt / 1 + this.settings.speed
	...
	player = {x: 0, x2: 0, lives: 3}
	scores = {score: 0, record_score: 0, coin: 0, diamond: 0}
	settings = {speed: 2.5, maxspeed: 1, colision: true}
	probs = {diamond: 5, coin: 80, live: 2}
