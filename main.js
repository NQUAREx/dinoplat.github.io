...
let theme_1 = {
	...
	block: {
		color: "red",
		background: "rgba(31.875, 31.875, 31.875, 0.5)",
		image: "none"
	}
}
...
class Game {
	...
	step(dt) {
		...
		for (let i = 0; i < this.blocks.length; ++i) {
			this.blocks[i][0] += dt / 2
			if (this.blocks[i][0] > canvas.height) {
				this.blocks[i][0] = -this.scale
				for (let x = 0; x < this.width; ++x) {
					this.blocks[i][x] = random(0, 1)
				}
			}
		}
	}
	...
	render() {
		...
		this.ctx.fillRect((this.player.x2 * this.scale), (this.canvas.height - this.scale * (this.width - 1)), this.scale, this.scale)

		this.ctx.fillStyle = game.theme.block.color
		for (let i = 0; i < this.blocks.length; ++i) {
			for (let x = 0; x < this.blocks[i].length; ++x) {
				if (this.blocks[i][x] == 1) this.ctx.fillRect((x - 1) * this.scale, this.blocks[i][0], this.scale, this.scale)
			}
		}
	}
	...
	score = 0
	blocks = [[0, 1, 0, 0, 0, 1], [50, 1, 0, 0, 0, 1], [100, 1, 0, 0, 0, 1], [150, 1, 0, 0, 0, 1], [200, 1, 0, 0, 0, 1]]
	// [50, 1, 1, 0, 0, 0, 1]
}

let game = new Game(loop, canvas, 5, theme_1)
let time
